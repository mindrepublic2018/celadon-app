  ...
  return (
    <div style={{ height:"100%", overflowY:"auto", paddingBottom:90, background:"linear-gradient(180deg, #0a0a0a 0%, #0d1a12 100%)" }}>
      ...
      <div style={{ padding:"20px 20px 0" }}>
        ...
        {records.map((item,i)=>(<div key={item.id} onClick={()=>{if(item.payment_status==="완료") onViewCertificate(item);}} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 16px",borderRadius:12,marginBottom:6,background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.03)",opacity:animate?1:0,transition:`opacity 0.4s ease ${0.15*i}s`,cursor:item.payment_status==="완료"?"pointer":"default"}}><div><div style={{fontSize:12,color:"#fff",fontWeight:600}}>{item.record_month}</div><div style={{fontSize:10,color:"rgba(255,255,255,0.25)",marginTop:2}}>{item.km}km 러닝</div></div><div style={{textAlign:"right"}}><div style={{fontSize:13,color:"#ACE1AF",fontFamily:"'Cormorant Garamond', serif",fontWeight:600}}>{(item.km*500).toLocaleString()}원</div><div style={{display:"flex",alignItems:"center",gap:4,justifyContent:"flex-end",marginTop:2}}>{item.payment_status==="완료"&&<span style={{fontSize:8,color:"rgba(172,225,175,0.4)"}}>📜 증서보기</span>}<div style={{fontSize:8,color:item.payment_status==="완료"?"rgba(172,225,175,0.5)":"rgba(255,215,0,0.5)",padding:"1px 6px",borderRadius:8,background:"rgba(172,225,175,0.06)",display:"inline-block"}}>{item.payment_status}</div></div></div></div>))}
      </div>
...
function CertificateScreen({ record, profile, setScreen }) {
  const [saving, setSaving] = useState(false);
  const canvasRef = useState(null);
  const month = record.record_month; // "2026-02"
  const [y, m] = month.split('-');
  const lastDay = new Date(Number(y), Number(m), 0).getDate();
  const startDate = `${y}.${m}.01`;
  const endDate = `${y}.${m}.${String(lastDay).padStart(2,'0')}`;
  const donation = (Number(record.km)*500).toLocaleString();
  const fullName = profile?.full_name || profile?.name || "러너";
  const issueDate = `${y}년 ${Number(m)}월 ${lastDay}일`;
  ...
  const drawCertificate = (canvas) => {
    ...
    ctx.fillText('Celadon Donation Running', W/2, 110);

    ctx.fillStyle = '#333';
    ctx.font = 'bold 42px "Noto Sans KR", sans-serif';
    ctx.fillText('기부증서', W/2, 175);

    // 이름
    ctx.textAlign = 'right';
    ctx.font = 'bold 24px "Noto Sans KR", sans-serif';
    ctx.fillStyle = '#222';
    ctx.fillText(`${fullName}님`, W-80, 260);
    ...
    ctx.fillStyle = '#444';
    const lines = [
      '중동 난민들을 위한 스포츠 학교 건립을 목표로 한',
      '10만 km 달리기 "이유 있는 러닝"에 참여해 주셔서',
      '진심으로 감사드립니다. 당신의 이유 있는 러닝은',
      '세상을 더 따뜻하게 만들었습니다.'
    ];
    lines.forEach((line, i) => {
      ctx.fillText(line, W/2, 340 + i*36);
    });

    // 참여 기간 & 기부금액
    ctx.font = '15px "Noto Sans KR", sans-serif';
    ctx.fillStyle = '#666';
    ctx.fillText(`참여 기간 (${startDate} ~ ${endDate})`, W/2, 540);
    ctx.fillText(`기부금액 ${donation}원`, W/2, 568);

    // 날짜
    ctx.font = 'bold 20px "Noto Sans KR", sans-serif';
    ctx.fillStyle = '#333';
    ctx.fillText(issueDate, W/2, 650);

    // 셀라돈 대표
    ctx.font = '16px "Noto Sans KR", sans-serif';
    ctx.fillStyle = '#666';
    ctx.fillText('셀라돈 대표', W/2 - 60, 740);
    ctx.font = 'bold 28px "Noto Sans KR", serif';
    ctx.fillStyle = '#333';
    ctx.fillText('정  영  민', W/2 + 40, 742);

    // 도장 (원형)
    ctx.beginPath();
    ctx.arc(W/2 + 140, 730, 35, 0, Math.PI*2);
  ...
  const handleSave = () => {
    ...
    link.download = `기부증서_${fullName}_${month}.png`;
  ...
  return (
    <div style={{ height:"100%", overflowY:"auto", background:"linear-gradient(180deg, #0a0a0a 0%, #0d1a12 100%)" }}>
      <div style={{ padding:"50px 24px 8px", display:"flex", alignItems:"center", gap:12 }}>
        <button onClick={()=>setScreen(SCREENS.MYPAGE)} style={{ background:"none", border:"none", cursor:"pointer", color:"rgba(255,255,255,0.4)", fontSize:20, padding:"4px 8px" }}>←</button>
        <div><div style={{ color:"rgba(172,225,175,0.4)", fontSize:10, fontFamily:"'Space Mono', monospace", letterSpacing:3 }}>CERTIFICATE</div><div style={{ color:"#fff", fontSize:18, fontFamily:"'Cormorant Garamond', serif", fontWeight:600, marginTop:2, letterSpacing:1 }}>기부증서</div></div>
      </div>
      <div style={{ padding:"16px 20px" }}>
        <div style={{ borderRadius:16, overflow:"hidden", background:"#fff", boxShadow:"0 4px 30px rgba(0,0,0,0.3)" }}>
          <canvas id="cert-canvas" style={{ width:"100%", height:"auto", display:"block" }}/>
        </div>
        <div style={{ display:"flex", gap:10, marginTop:16 }}>
          <button onClick={handleSave} style={{ flex:1, padding:"14px", borderRadius:14, border:"none", cursor:"pointer", background:"linear-gradient(135deg, #ACE1AF, #8FBC8F)", fontSize:13, fontWeight:700, color:"#0a0a0a" }}>📥 이미지 저장</button>
          <button onClick={()=>{const canvas=document.getElementById('cert-canvas');if(!canvas)return;canvas.toBlob(blob=>{if(navigator.share){navigator.share({files:[new File([blob],`기부증서_${fullName}.png`,{type:'image/png'})],title:'셀라돈 기부증서'}).catch(()=>{});}else{handleSave();}});}} style={{ flex:1, padding:"14px", borderRadius:14, border:"1px solid rgba(172,225,175,0.2)", cursor:"pointer", background:"transparent", fontSize:13, fontWeight:600, color:"#ACE1AF" }}>📤 공유하기</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      <div style={{ width:"100%", maxWidth:390, height:"100vh", maxHeight:844, margin:"0 auto", position:"relative", overflow:"hidden", background:"#0a0a0a", fontFamily:"'Noto Sans KR', sans-serif", borderRadius:typeof window!=='undefined'&&window.innerWidth>500?40:0, boxShadow:"0 0 100px rgba(172,225,175,0.03)" }}>
        ...
        {screen===SCREENS.MYPAGE && <MyPageScreen userData={userData} setScreen={handleSetScreen} profile={profile} onLogout={handleLogout} onViewCertificate={(record)=>{setCertRecord(record);setScreen(SCREENS.CERTIFICATE);}} />}
        {screen===SCREENS.CERTIFICATE && certRecord && <CertificateScreen record={certRecord} profile={profile} setScreen={handleSetScreen} />}
